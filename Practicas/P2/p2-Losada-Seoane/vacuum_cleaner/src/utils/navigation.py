...
class Navigation():
    ...
    def __init__(self, id, pose):
        ...
        self.goal_pose.x = pose[0]
        self.goal_pose.y = pose[1]
        self.rotations = pose[2]

        # Starts a new node
        rospy.init_node('rl_robot', anonymous=True)

        # Publishers
        self.velocity_publisher = rospy.Publisher('/Robot' + str(id) + '/cmd_vel', Twist, queue_size=10)
        

        # Subscriptions
        rospy.Subscriber('/Robot' + str(id) + '/odom',Odometry,self.odometryCb)
    ...
    def getPos(self):
        return self.goal_pose.x, self.goal_pose.y, self.rotations 
